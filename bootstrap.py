...
def buscar_por_dni(dni):
    with get_connection() as conn:
        row = conn.execute("SELECT * FROM trabajadores WHERE dni = ?", (dni.strip(),)).fetchone()
    return dict(row) if row else None

def listar_activos():
    with get_connection() as conn:
        rows = conn.execute("SELECT * FROM trabajadores WHERE estado = \'ACTIVO\' ORDER BY nombre ASC").fetchall()
    return [dict(r) for r in rows]

def cambiar_estado(dni, nuevo_estado):
    if nuevo_estado not in _ESTADOS_VALIDOS:
        raise ValueError(f"Estado invalido: {nuevo_estado}")
    with get_connection() as conn:
        cursor = conn.execute("UPDATE trabajadores SET estado = ? WHERE dni = ?", (nuevo_estado, dni.strip()))
    ...
    with get_connection() as conn:
        cursor = conn.execute(
            """INSERT OR IGNORE INTO trabajadores
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (datos["dni"], datos["nombre"], datos.get("apellido"), datos.get("cargo"),
             datos.get("fecha_nacimiento"), datos.get("correo"), datos.get("celular"),
...
def importar_trabajadores(ruta_excel):
    ...
    try:
        for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            try:
                ...
                if not dni or dni == "None": omitidos += 1; continue
                nombre_raw = str(nombre_col).strip() if nombre_col else ""
                if not nombre_raw or nombre_raw == "None": omitidos += 1; continue
                ...
                created_at = datetime.now().strftime(\'%Y-%m-%d %H:%M:%S\')
                cursor = conn.execute(_INSERT, (dni, nombre, apellido_str, cargo_str, fecha_str, correo_str, celular_str, estado_val, created_at))
                if cursor.rowcount == 1: importados += 1
                else: omitidos += 1
            except Exception as exc:
                errores.append(f"Fila {row_idx}: {exc}"); omitidos += 1
        conn.commit()
    finally:
        conn.close(); wb.close()
...
ON CONFLICT(dni) DO UPDATE SET
    ...
    fecha_nacimiento=excluded.fecha_nacimiento, correo=excluded.correo,
    celular=excluded.celular, estado=excluded.estado
"""

def sincronizar_desde_sheets():
    url = f"https://docs.google.com/spreadsheets/d/{GOOGLE_SHEET_ID}/gviz/tq?tqx=out:csv&sheet=Registro"
    ...
    nuevos=actualizados=omitidos=0; errores=[]; conn=get_connection()
    try:
        for i, fila in enumerate(lineas[1:], start=2):
            ...
            except Exception as exc: errores.append(f"Fila {i}: {exc}")
        conn.commit()
    finally: conn.close()
...
def _aplicar_fuente(doc):
    for p in doc.paragraphs:
        for r in p.runs: r.font.name=\'Century Gothic\'; r.font.size=Pt(11)
    for t in doc.tables:
        for row in t.rows:
...
def generar_constancia(dni, nombre, apellido, cargo, fecha):
    try:
        ...
        contexto={\'NOMBRE\':nombre,\'APELLIDOS\':apellido or nombre,\'DNI\':dni,\'CARGO\':cargo or \'\',\'FECHA\':convertir_fecha(fecha)}
        if ruta_firma.exists(): contexto[\'FIRMA\']=InlineImage(doc,str(ruta_firma),width=Cm(4))
        doc.render(contexto); _aplicar_fuente(doc)
        nombre_docx=f"constancia_RISST_{nombre.replace(\' \',\'_\')}.docx"
        ruta_docx=CARPETA_SALIDA/nombre_docx
        doc.save(str(ruta_docx)); convert(str(ruta_docx))
        nombre_pdf=nombre_docx.replace(\'.docx\',\'.pdf\')
        with get_connection() as conn:
            conn.execute("INSERT INTO documentos_generados (dni_trabajador,tipo_documento,fecha_generacion,ruta_archivo) VALUES (?,?,?,?)",
                (dni,\'CONSTANCIA_RISST\',datetime.now().strftime(\'%Y-%m-%d %H:%M:%S\'),str(CARPETA_SALIDA/nombre_pdf)))
        return True, nombre_pdf
    except Exception as exc: return False, str(exc)

def generar_todas_constancias(fecha):
    with get_connection() as conn:
        rows=conn.execute("SELECT dni,nombre,apellido,cargo FROM trabajadores WHERE estado=\'ACTIVO\' ORDER BY nombre ASC").fetchall()
...
def procesar_firma(ruta_entrada, ruta_salida):
    with open(ruta_entrada,\'rb\') as f: datos=f.read()
    output=remove(datos)
    imagen=Image.open(io.BytesIO(output)).convert("RGBA")
    ancho_orig,alto_orig=imagen.size
    proporcion=ALTO_FINAL/alto_orig; ancho_final=int(ancho_orig*proporcion)
    imagen.resize((ancho_final,ALTO_FINAL),Image.Resampling.LANCZOS).save(str(ruta_salida),format=\'PNG\')

def procesar_todas_firmas():
    ...
    for archivo in sorted(CARPETA_ENTRADA.iterdir()):
        if archivo.suffix.lower() not in _EXTENSIONES: continue
        ruta_salida=CARPETA_SALIDA/(archivo.stem+\'.png\')
        if ruta_salida.exists(): omitidas+=1; continue
        try: procesar_firma(archivo,ruta_salida); procesadas+=1
        except Exception as exc: errores.append(f"{archivo.name}: {exc}")
...
def _scrolled_text(parent, height):
    ...
    widget.pack(fill=\'both\',expand=True); return widget
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("SGSST - MARA"); self.geometry("720x580"); self.resizable(False,False)
        s=ttk.Style()
        for w in (\'TLabel\',\'TButton\',\'TEntry\',\'TCombobox\'): s.configure(w,font=(\'Segoe UI\',10))
        nb=ttk.Notebook(self); nb.pack(fill=\'both\',expand=True,padx=10,pady=10)
        self._build_tab_trabajadores(nb); self._build_tab_constancias(nb); self._build_tab_firmas(nb)
        init_db()

    def _build_tab_trabajadores(self,nb):
        tab=ttk.Frame(nb,padding=15); nb.add(tab,text="  Trabajadores  ")
        acc=ttk.LabelFrame(tab,text="Importar / Sincronizar",padding=10); acc.pack(fill=\'x\',pady=(0,8))
        ...
        srch=ttk.LabelFrame(tab,text="Consultar",padding=10); srch.pack(fill=\'x\',pady=(0,8))
        ...
        res=ttk.LabelFrame(tab,text="Resultados",padding=10); res.pack(fill=\'both\',expand=True)
    ...
    def _sync_sheets(self):
        def run():
            ...
            except Exception as exc: self.after(0,lambda: messagebox.showerror("Error",str(exc)))
        threading.Thread(target=run,daemon=True).start()

    def _importar_excel(self):
        ruta=filedialog.askopenfilename(title="Seleccionar Excel",filetypes=[(\'Excel\',\'*.xlsx *.xls\')])
        if not ruta: return
        try:
    ...
    def _build_tab_constancias(self,nb):
        tab=ttk.Frame(nb,padding=15); nb.add(tab,text="  Constancias RISST  ")
        cfg=ttk.LabelFrame(tab,text="Configuracion",padding=10); cfg.pack(fill=\'x\',pady=(0,8))
        ...
        self._prog_const=ttk.Progressbar(tab,mode=\'indeterminate\',length=500); self._prog_const.pack(fill=\'x\',pady=(0,8))
        log=ttk.LabelFrame(tab,text="Progreso",padding=10); log.pack(fill=\'both\',expand=True)
    ...
    def _generar_constancias(self):
        ...
        _write(self._txt_constancias,""); self._prog_const.start()
        def run():
            try:
                res=generar_todas_constancias(fecha)
                for line in res[\'log\']: self.after(0,lambda l=line: _write(self._txt_constancias,l+\'\\n\',clear=False))
                self.after(0,lambda: _write(self._txt_constancias,f"\\n{\'=\'*50}\\nTotal:{res[\'total\']} | OK:{res[\'exitosos\']} | Errores:{res[\'fallidos\']}",clear=False))
                self.after(0,lambda: messagebox.showinfo("Completado",f"Generacion finalizada.\\n{res[\'exitosos\']} de {res[\'total\']} constancias generadas."))
            except Exception as exc: self.after(0,lambda: messagebox.showerror("Error",str(exc)))
            finally: self.after(0,self._prog_const.stop)
        threading.Thread(target=run,daemon=True).start()

    def _build_tab_firmas(self,nb):
        tab=ttk.Frame(nb,padding=15); nb.add(tab,text="  Firmas  ")
        info=ttk.LabelFrame(tab,text="Instrucciones",padding=10); info.pack(fill=\'x\',pady=(0,10))
        ttk.Label(info,text=(
        ...
        self._prog_firmas=ttk.Progressbar(tab,mode=\'indeterminate\',length=500); self._prog_firmas.pack(fill=\'x\',pady=(0,8))
        log=ttk.LabelFrame(tab,text="Progreso",padding=10); log.pack(fill=\'both\',expand=True)
        self._txt_firmas=_scrolled_text(log,14)

    def _procesar_firmas(self):
        _write(self._txt_firmas,""); self._prog_firmas.start()
        def run():
            try:
                ...
                self.after(0,lambda: messagebox.showinfo("Completado",f"Procesadas: {res[\'procesadas\']}\\nOmitidas: {res[\'omitidas\']}\\nErrores: {len(res[\'errores\'])}"))
            except Exception as exc: self.after(0,lambda: messagebox.showerror("Error",str(exc)))
            finally: self.after(0,self._prog_firmas.stop)
        threading.Thread(target=run,daemon=True).start()
